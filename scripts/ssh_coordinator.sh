...
# Hardcoded configuration (reuse values from ssh_backend.sh)
...
echo "[INFO] Looking up CoordinatorLaunchTemplate in stack: ${STACK_NAME}"
...
LT_ID=$(aws cloudformation describe-stack-resources \
  ...
  --logical-resource-id CoordinatorLaunchTemplate \
  --query 'StackResources[0].PhysicalResourceId' \
  --output text \
  --region "${REGION}")

if [[ -z "${LT_ID}" || "${LT_ID}" == "None" ]]; then
  echo "[ERROR] Could not find CoordinatorLaunchTemplate in stack ${STACK_NAME}."
  exit 1
fi

echo "[INFO] Launch Template ID: ${LT_ID}"

# Find coordinator instance by tags
echo "[INFO] Looking for coordinator instances with Type=coordinator tag..."
...
if [[ -z "${INSTANCE_ID}" || "${INSTANCE_ID}" == "None" ]]; then
  echo "[ERROR] No running coordinator instances found."
  echo "[INFO] This might mean:"
  echo "  - Coordinator instances are still starting up"
  echo "  - Instances don't have the required tags"
  echo "  - No coordinator instances are running"
  exit 1
fi
