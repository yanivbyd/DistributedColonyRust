...
main() {
    ...
    CLOUD_START_URL="http://${COORDINATOR_IP}:${HTTP_PORT}/cloud-start?idempotency_key=${IDEMPOTENCY_KEY}"
    ...
    if [ "$CURL_EXIT_CODE" -ne 0 ]; then
        print_error "Failed to connect to cloud-start endpoint"
    ...
    if [ "$HTTP_CODE" -eq 200 ]; then
        ...
        if echo "$RESPONSE_BODY" | grep -q "idempotent"; then
            print_status "Cloud-start already completed with same idempotency_key (HTTP 200 - idempotent)"
            if [ -s "$TEMP_RESPONSE" ]; then
                log_output "Response:"
                cat "$TEMP_RESPONSE" | sed 's/^/  /' | tee -a "${LOG_FILE:-/dev/stdout}"
            fi
        else
            print_status "Cloud-start succeeded (HTTP 200)"
            if [ -s "$TEMP_RESPONSE" ]; then
                log_output "Response:"
                cat "$TEMP_RESPONSE" | sed 's/^/  /' | tee -a "${LOG_FILE:-/dev/stdout}"
            fi
        ...
        print_status "Cloud-start triggered successfully! (HTTP 202 Accepted)"
        if [ -s "$TEMP_RESPONSE" ]; then
            log_output "Response:"
            cat "$TEMP_RESPONSE" | sed 's/^/  /' | tee -a "${LOG_FILE:-/dev/stdout}"
        fi
