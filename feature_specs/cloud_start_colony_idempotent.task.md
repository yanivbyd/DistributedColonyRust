...
6. Unit tests should be added to verify this behavior
...
### 3. Unit Tests (`crates/coordinator/tests/test_cloud_start_colony.rs`)

**New Tests Needed:**

1. **Test: cloud-start fails when colony already initialized with different idempotency_key**
   - Setup: Initialize CoordinatorContext with status set to `ColonyStatus::TopographyInitialized` and `cloud_start_idempotency_key` set to `Some("key1")` (colony initialized with key1)
   - Action: Call POST /cloud-start endpoint with idempotency_key "key2"
   - Assert: HTTP response is 409 Conflict with "Colony already started" message

2. **Test: cloud-start succeeds (idempotent) when colony already initialized with same idempotency_key**
   - Setup: Initialize CoordinatorContext with status set to `ColonyStatus::TopographyInitialized` and `cloud_start_idempotency_key` set to `Some("key1")` (colony initialized with key1)
   - Action: Call POST /cloud-start endpoint with idempotency_key "key1"
   - Assert: HTTP response is 200 OK with "Colony already started (idempotent)" message

3. **Test: cloud-start succeeds when colony not initialized**
   - Setup: CoordinatorContext initialized with status set to `ColonyStatus::NotInitialized`
   - Action: Call POST /cloud-start endpoint with idempotency_key "key1"
   - Assert: HTTP response is 202 Accepted
   - Verify: idempotency_key "key1" is stored in memory

4. **Test: cloud-start idempotency_key is stored after successful initialization**
   - Setup: CoordinatorContext initialized with status set to `ColonyStatus::NotInitialized` and `cloud_start_idempotency_key` set to `None`
   - Action: Call POST /cloud-start endpoint with idempotency_key "key1", wait for completion
   - Assert: `cloud_start_idempotency_key` is set to `Some("key1")` in CoordinatorStoredInfo and persisted to disk
   - Action: Call POST /cloud-start endpoint again with idempotency_key "key1"
   - Assert: HTTP response is 200 OK (idempotent)

**Test Infrastructure:**
- May need to add helper functions to set up CoordinatorContext with colony initialized vs not initialized
- May need to add HTTP client helper to test the HTTP endpoint directly
- Need to test idempotency_key storage and retrieval from CoordinatorStoredInfo
- Need to verify idempotency_key persistence across coordinator restarts
...
4. `crates/coordinator/tests/test_cloud_start_colony.rs`
   ...
   - May need to add HTTP endpoint testing utilities
...
- [ ] Unit test: cloud-start fails when colony already initialized with different idempotency_key
- [ ] Unit test: cloud-start succeeds (idempotent) when colony already initialized with same idempotency_key
- [ ] Unit test: cloud-start succeeds when colony not initialized
- [ ] Unit test: cloud-start idempotency_key is stored in CoordinatorStoredInfo after successful initialization
- [ ] Unit test: cloud-start succeeds when colony status is NotInitialized (colony not yet initialized)
- [ ] Manual test: Run cloud-start script when colony already started with different idempotency_key, verify it exits with error
- [ ] Manual test: Run cloud-start script when colony already started with same idempotency_key, verify it succeeds (idempotent)
- [ ] Manual test: Run cloud-start script when colony not started, verify it succeeds
- [ ] Verify bash script exits with non-zero code on "Colony already started" error (409)
- [ ] Verify bash script treats idempotent success (200) as success
